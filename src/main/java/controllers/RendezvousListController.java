...
public class RendezvousListController {
    ...
    private void renderCards() {
        ...
        if (filtered.isEmpty()) {
            Label empty = new Label("Aucun rendez-vous trouvÃ©.");
    ...
    private VBox buildCard(Rendezvous rdv) {
        ...
        switch (rdv.getStatus()) {
            case "CONFIRME" -> {
                ...
                badgeText = "#16a34a"; statusLabel = "âœ… ConfirmÃ©";
            }
            case "ANNULE" -> {
                borderColor = "#ef4444"; badgeBg = "#fee2e2";
                badgeText = "#dc2626"; statusLabel = "âŒ AnnulÃ©";
            }
            case "TERMINE" -> {
                borderColor = "#8b5cf6"; badgeBg = "#ede9fe";
                badgeText = "#7c3aed"; statusLabel = "ðŸ TerminÃ©";
            }
            default -> {
                borderColor = "#f59e0b"; badgeBg = "#fef3c7";
                badgeText = "#d97706"; statusLabel = "â³ En attente";
            }
        }

        // âœ… Parser la description
        ...
        Label phoneLabel = new Label("ðŸ“ž  " + (rdv.getNum() != 0 ? rdv.getNum() : "-"));
        ...
        // Ligne 2 : Animal + CrÃ©neau
        HBox infoRow = new HBox(20);
        infoRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);

        Label animalLabel = new Label("ðŸ¾  " + (animalInfo.isEmpty() ? "-" : animalInfo));
        animalLabel.setStyle("-fx-font-size: 13; -fx-font-weight: bold; -fx-text-fill: #92400e;");

        Label slotLabel = new Label("ðŸ“…  " + (slotInfo.isEmpty() ? "-" : slotInfo));
        ...
        if ("EN_ATTENTE".equals(rdv.getStatus())) {
            ...
            Button btnRefuser = new Button("âŒ Refuser");
            btnRefuser.setStyle(
        ...
        // âœ… Bouton Terminer si CONFIRME
        if ("CONFIRME".equals(rdv.getStatus())) {
            HBox btnRow = new HBox(10);
            btnRow.setAlignment(javafx.geometry.Pos.CENTER_RIGHT);

            Button btnTerminer = new Button("ðŸ Consultation terminÃ©e");
    ...
    private void updateStatus(Rendezvous rdv, String newStatus) {
        try {
            ...
            if (!clientNum.equals("0") && !clientNum.isEmpty()) {
                ...
                System.out.println("ðŸ“± Envoi SMS Ã  : " + phone);
    ...
    private void onBackHome(ActionEvent event) {
        ViewNavigator.goTo(event, "/Dashboard.fxml", "Dashboard VÃ©tÃ©rinaire");
    }
    @FXML
