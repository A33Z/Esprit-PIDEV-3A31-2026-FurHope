...
public class StatsDashboardController {
    ...
    private void loadDashboard() {
        try {
            ...
            String evolution = diff > 0 ? "â†‘ +" + diff : diff < 0 ? "â†“ " + diff : "â†’ Stable";
            ...
            Platform.runLater(() -> {
                ...
                // âœ… Charger analyse IA aprÃ¨s chargement page
    ...
    private String buildHtml(
        ...
        String etoiles = "â­".repeat((int) Math.round(noteMoy)) +
                "â˜†".repeat(5 - (int) Math.round(noteMoy));
        ...
        return "<!DOCTYPE html><html><head>" +
                ...
                // âœ… Cartes stats
                "<div class='grid'>" +
                "<div class='card'><div class='card-title'>âœ… ConfirmÃ©s</div>" +
                "<div class='card-value' style='color:#22c55e;'>" + confirme + "</div>" +
                "<div class='card-sub'>ce mois</div></div>" +

                "<div class='card'><div class='card-title'>âŒ AnnulÃ©s</div>" +
                "<div class='card-value' style='color:#ef4444;'>" + annule + "</div>" +
                "<div class='card-sub'>ce mois</div></div>" +

                "<div class='card'><div class='card-title'>â³ En attente</div>" +
                "<div class='card-value' style='color:#f59e0b;'>" + attente + "</div>" +
                "<div class='card-sub'>ce mois</div></div>" +

                "<div class='card'><div class='card-title'>ðŸ“ˆ Ã‰volution</div>" +
                "<div class='card-value' style='color:" + evolutionColor + ";font-size:22px;'>" + evolution + "</div>" +
                "<div class='card-sub'>vs mois prÃ©cÃ©dent</div></div>" +
                "</div>" +

                // âœ… Note moyenne
                "<div class='grid' style='grid-template-columns:1fr 1fr;'>" +
                "<div class='card'><div class='card-title'>â­ Note moyenne</div>" +
                ...
                "<div class='card'><div class='card-title'>ðŸ TerminÃ©s</div>" +
                ...
                // âœ… Analyse IA
                "<div class='ai-card'>" +
                "<div class='ai-title'>ðŸ¤– Analyse IA â€” Conseil personnalisÃ©</div>" +
                "<div id='aiText'>â³ Analyse en cours...</div>" +
                "</div>" +


                "<div class='chart-card'>" +
                "<div class='chart-title'>ðŸ“… Rendez-vous par jour â€” ce mois</div>" +
                "<canvas id='rdvChart' height='100'></canvas></div>" +


                "<div class='chart-card'>" +
                "<div class='chart-title'>â­ Ã‰volution de vos notes clients</div>" +
                "<canvas id='reviewChart' height='100'></canvas></div>" +

                "<div class='chart-card'>" +
                "<div class='chart-title'>ðŸ“Š Comparaison avec les autres vÃ©tÃ©rinaires</div>" +
    ...
    private void loadAiConseil(int confirme, int annule, int attente, int totalMoisPrec, int vetId) {
        CompletableFuture.runAsync(() -> {
            try {
                ...
                String prompt;
                if (count == 0) {
                    prompt = "Tu es un assistant pour vÃ©tÃ©rinaires. " +
                            "Statistiques du Dr. " + SessionManager.getUserNom() + " ce mois : " +
                            "ConfirmÃ©s=" + confirme + ", AnnulÃ©s=" + annule + ", En attente=" + attente +
                            ". Mois prÃ©cÃ©dent total=" + totalMoisPrec +
                            ". Donne 2-3 conseils professionnels pour amÃ©liorer sa performance. En franÃ§ais.";
                } else {
                    prompt = "Tu es un expert satisfaction client vÃ©tÃ©rinaire. " +
                            "Avis clients du Dr. " + SessionManager.getUserNom() + " : " + commentaires +
                            "Stats ce mois: ConfirmÃ©s=" + confirme + ", AnnulÃ©s=" + annule +
                            ". Donne: 1) Points forts 2) Points Ã  amÃ©liorer 3) Conseil concret. " +
                            "Max 4 phrases. En franÃ§ais.";
                }
                ...
                Platform.runLater(() ->
                        engine.executeScript(
                                "document.getElementById('aiText').innerHTML = 'âŒ Erreur IA : " +
    ...
    private String callGroqApi(String prompt, int maxTokens) throws Exception {
        ...
        try (okhttp3.Response response = client.newCall(request).execute()) {
            ...
            if (result.has("error")) {
                return "âŒ " + result.getAsJsonObject("error")
    ...
    private void logout(ActionEvent event) {
        ...
        ViewNavigator.goTo(event, "/Home.fxml", "Clinique VÃ©tÃ©rinaire");
    }
}