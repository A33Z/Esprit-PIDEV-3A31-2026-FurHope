...
public class feedcontroller {
    ...
    private final ApiPostService postSvc = new ApiPostService();
    private final ApiCommentService commentSvc = new ApiCommentService();
    ...
    private void onNewPostClick() {
        ...
        postBtn.setOnAction(e -> {
            ...
            Thread thread = new Thread(createTask, "post-create-api");
    ...
    private void loadPosts(Runnable onSuccess) {
        ...
        task.setOnFailed(event -> {
            ...
            ex.printStackTrace();
            new Alert(Alert.AlertType.ERROR, "Failed to load feed from API: " + ex.getMessage()).showAndWait();
        });

        Thread thread = new Thread(task, "feed-api-load");
        thread.setDaemon(true);
        thread.start();
    }
    ...
    private void scrollToPost(long postId) {
        ...
        for (Node node : postsbox.getChildren()) {
            ...
            feedScrollPane.setVvalue(target);
            break;
        }
    }
