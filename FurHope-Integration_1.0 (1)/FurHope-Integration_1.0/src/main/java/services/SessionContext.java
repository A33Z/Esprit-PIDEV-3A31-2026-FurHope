...
public final class SessionContext {
    ...
    public static synchronized void startUserSession(int userId) {
        ...
        User user = resolveUserFromDatabase(userId);
    ...
    private static User resolveUserFromDatabase(int userId) {
        try {
            User dbUser = new userservices().findById(userId);
            if (dbUser != null) {
                if (dbUser.getManagerId() == null || dbUser.getManagerId().isBlank()) {
                    dbUser.setManagerId("USER-" + dbUser.getId());
                }
                if (dbUser.getRole() == null || dbUser.getRole().isBlank()) {
                    dbUser.setRole(Role.USER.name());
                }
                return dbUser;
            }
        } catch (SQLException ignored) {
            // Fallback keeps login flow available if database lookup fails unexpectedly.
        }
        return new User(userId, "User #" + userId, Role.USER, "USER-" + userId);
    }
