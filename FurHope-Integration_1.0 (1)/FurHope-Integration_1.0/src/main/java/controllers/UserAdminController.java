...
public class UserAdminController {
    ...
    private VBox buildUserCard(User user) {
        ...
        Label addressLabel = new Label("Address: " + safeText(user.getAddress()));
        ...
        Label statusLabel = new Label("Account status: " + (user.isActive() ? "Active" : "Pending/Blocked"));
        statusLabel.getStyleClass().add("card-subtitle");

        Button activateBtn = new Button("Activate");
        activateBtn.getStyleClass().addAll("primary-button", "compact-action-button");
        activateBtn.setDisable(user.isActive());
        activateBtn.setOnAction(e -> updateUserActive(user, true));
        activateBtn.setMaxWidth(Double.MAX_VALUE);
        HBox.setHgrow(activateBtn, Priority.ALWAYS);

        Button blockBtn = new Button("Block");
        blockBtn.getStyleClass().addAll("secondary-button", "compact-action-button");
        blockBtn.setDisable(!user.isActive());
        blockBtn.setOnAction(e -> updateUserActive(user, false));
        blockBtn.setMaxWidth(Double.MAX_VALUE);
        HBox.setHgrow(blockBtn, Priority.ALWAYS);

        Button deleteBtn = new Button("Delete");
        deleteBtn.getStyleClass().addAll("ghost-button", "compact-action-button");
        deleteBtn.setOnAction(e -> deleteUser(user));
        deleteBtn.setMaxWidth(Double.MAX_VALUE);
        HBox.setHgrow(deleteBtn, Priority.ALWAYS);

        HBox actions = new HBox(10, activateBtn, blockBtn, deleteBtn);
        actions.setAlignment(Pos.CENTER_LEFT);

        VBox card = new VBox(6, nameLabel, emailLabel, phoneLabel, addressLabel, roleLabel, statusLabel, actions);
    ...
    private void updateUserActive(User user, boolean active) {
        if (user == null) {
            return;
        }
        if (isAdminUser(user)) {
    ...
    private void renderUserCards(ObservableList<User> users) {
        if (userCardsPane == null) {
            return;
        }
        userCardsPane.getChildren().clear();
        for (User user : users) {
            userCardsPane.getChildren().add(buildUserCard(user));
        }
    }
}
